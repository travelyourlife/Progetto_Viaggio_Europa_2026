...
(function () {
  ...
  // v4.63: three-language detection
  function lang3() {
    if (typeof window.LANG3 === 'string') return window.LANG3;
    var p = (location.pathname || '').toLowerCase();
    var l = (document.documentElement.getAttribute('lang') || '').toLowerCase();
    if (p.indexOf('_es') !== -1 || l.indexOf('es') === 0) return 'es';
    if (p.indexOf('_en') !== -1 || l.indexOf('en') === 0) return 'en';
    return 'it';
  }

  var T = {
    title:     { it: '🧭 Itinerari città',           en: '🧭 City itineraries', es: '🧭 Itinerarios urbanos' },
    subtitle:  { it: 'Percorsi a piedi nelle città principali del viaggio. Tocca una tappa per leggere tutto; apri la mappa per orientarti e raggiungere i luoghi.',
                 ...
                 es: 'Rutas a pie por las principales ciudades del viaje. Toca una parada para leer más; abre el mapa para orientarte y llegar a cada lugar.' },
    openMap:   { it: '🗺️ Apri mappa itinerario',     en: '🗺️ Open itinerary map', es: '🗺️ Abrir mapa del itinerario' },
    closeMap:  { it: '✖ Chiudi mappa',               en: '✖ Close map', es: '✖ Cerrar mapa' },
    readMore:  { it: 'Leggi tutto ▾',                en: 'Read more ▾', es: 'Leer más ▾' },
    readLess:  { it: 'Riduci ▴',                     en: 'Show less ▴', es: 'Mostrar menos ▴' },
    stop:      { it: 'Tappa',                        en: 'Stop', es: 'Parada' },
    walk:      { it: '🚶 A piedi',                   en: '🚶 Walk', es: '🚶 A pie' },
    scooter:   { it: '🛴 Monopattino',               en: '🛴 Scooter', es: '🛴 Patinete' },
    transit:   { it: '🚍 Mezzi',                     en: '🚍 Transit', es: '🚍 Transporte' },
    sources:   { it: 'Fonti',                        en: 'Sources', es: 'Fuentes' },
    youHere:   { it: '📍 Sei qui!',                  en: '📍 You are here!', es: '📍 ¡Estás aquí!' },
    locating:  { it: 'Individuazione posizione…',    en: 'Locating you…', es: 'Localizando…' },
    locOff:    { it: 'Posizione non disponibile (attiva il GPS).', en: 'Location unavailable (enable GPS).', es: 'Ubicación no disponible (activa el GPS).' },
    tips:      { it: 'ℹ️ Info pratiche',             en: 'ℹ️ Practical info', es: 'ℹ️ Info práctica' },
    routeNote: { it: 'Il tracciato collega le tappe in ordine: è una guida indicativa a piedi, non una navigazione stradale.',
                 ...
                 es: 'La línea conecta las paradas en orden: es una guía orientativa a pie, no una navegación paso a paso.' },
    noData:    { it: 'Itinerari in arrivo per altre città.', en: 'More city itineraries coming soon.', es: 'Próximamente más itinerarios urbanos.' },
    mustOnly:  { it: '⭐ Solo imperdibili',           en: '⭐ Must-see only', es: '⭐ Solo imprescindibles' },
    showAll:   { it: '↩︎ Mostra tutte',              en: '↩︎ Show all', es: '↩︎ Mostrar todo' },
    mustBadge: { it: 'Imperdibile',                  en: 'Must-see', es: 'Imprescindible' },
    recBadge:  { it: 'Consigliato',                  en: 'Recommended', es: 'Recomendado' },
    legend:    { it: 'Tocca i numeri sulla mappa. ⭐⭐ imperdibile, ⭐ consigliato.',
                 en: 'Tap the numbers on the map. ⭐⭐ must-see, ⭐ recommended.',
                 es: 'Toca los números del mapa. ⭐⭐ imprescindible, ⭐ recomendado.' },
    fsOpen:    { it: 'Schermo intero',               en: 'Fullscreen', es: 'Pantalla completa' },
    fsClose:   { it: 'Chiudi',                       en: 'Close', es: 'Cerrar' },
    openDetail:{ it: '📖 Apri dettaglio',              en: '📖 Open detail', es: '📖 Abrir detalle' }
  };
  function t(key) { var lg = lang3(); var e = T[key]; return e[lg] || e.en || e.it; }
  ...
  function ensureDataLoaded(cb) {
    ...
    s.src = './city-itineraries.js?v=5.58';
    s.defer = true;
