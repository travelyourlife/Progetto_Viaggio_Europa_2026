# Quo Vadis — Manuale Tecnico e Funzionale (Mermaid Inline)
...
La scelta di sviluppare Quo Vadis como Progressive Web App (PWA) anziché come applicazione nativa (APK per Android o pacchetto `.ipa` per iOS) è stata dettata da precise considerazioni tecniche e logistiche:
...
## 📊 11. Diagrammi di Architettura e Flusso Dati (Mermaid)

Di seguito vengono riportati i codici sorgente dei diagrammi in formato **Mermaid**. GitHub e molti viewer Markdown li renderizzeranno automaticamente come grafici vettoriali interattivi.
...
```mermaid
flowchart TD
    subgraph SOURCES["📡 Fonti di Posizione"]
        L1["📱 Livello 1: Browser Live<br/>(watchPosition)"]
        L2["🛣️ Livello 2: OSRM Routing<br/>(Stima Gap)"]
        L3["🚗 Livello 3: GPSLogger Android<br/>(Auto-Upload Drive)"]
        L4["📝 Livello 4: Contachilometri<br/>(Inserimento Manuale)"]
    end

    subgraph SYNC["🔄 Elaborazione & Sincronizzazione"]
        MERGE{"Merge & Deduplica<br/>(Timestamp 30s)"}
        PRIORITY{"Controllo Priorità<br/>odometerKm"}
    end

    subgraph STORE["🔥 Database (Firebase Realtime)"]
        RTDB_TRACKS["tracks/{date}/points"]
        RTDB_SUMM["dailySummaries/{date}"]
    end

    subgraph VIS["🗺️ Visualizzazione Utente"]
        MAP["🗺️ Mappa Leaflet<br/>(Traccia & Marker Live)"]
        STATS["📊 Widget Statistiche<br/>(Km Percorsi & Progressi)"]
    end

    L1 -->|1. Realtime JSON| MERGE
    L2 -->|2. GeoJSON stimato| MERGE
    L3 -->|3. File .gpx scaricato| MERGE
    
    MERGE -->|Traccia unificata| RTDB_TRACKS
    MERGE -->|Km calcolati GPS| PRIORITY
    
    L4 -->|4. Valore odometerKm| PRIORITY
    
    PRIORITY -->|odometerKm vince su GPS| RTDB_SUMM
    
    RTDB_TRACKS --> MAP
    RTDB_SUMM --> STATS
    
    style SOURCES fill:#e8eaf6,stroke:#283593,stroke-width:2px
    style SYNC fill:#fff3e0,stroke:#e65100,stroke-width:2px
    style STORE fill:#fff8e1,stroke:#f57f17,stroke-width:2px
    style VIS fill:#e0f2f1,stroke:#004d40,stroke-width:2px
```
...
```mermaid
flowchart TD
    subgraph CLIENT["🌐 Browser PWA (v1.60)"]
        UI["🎨 Interfaccia Utente<br/>(HTML5 / Tailwind CSS)"]
        APP["🧠 Logica Applicativa<br/>(app.js / days-renderer.js)"]
        SW["⚙️ Service Worker<br/>(sw.js - Offline Cache)"]
        DATA["📄 Single Source of Truth<br/>(days-data.js)"]
    end

    subgraph FIREBASE["🔥 Servizi Firebase"]
        AUTH["🔑 Firebase Auth<br/>(Google Login)"]
        RTDB["🔥 Realtime DB<br/>(Chat, Tracks, Stats)"]
        STORAGE["📁 Firebase Storage<br/>(Foto Diario & Chat)"]
        FUNCTIONS["⚡ Cloud Functions<br/>(Notifiche Push & Reminder)"]
        FCM["📬 Firebase Cloud Messaging"]
    end

    subgraph EXTERNAL["☁️ Servizi Esterni (API)"]
        DRIVE["📂 Google Drive<br/>(file GPX)"]
        OSRM["🛣️ OSRM<br/>(routing stradale)"]
        WEATHER["🌤️ Open-Meteo<br/>(previsioni)"]
        NOMINATIM["📍 Nominatim<br/>(reverse geocoding)"]
        TILES["🗺️ OpenStreetMap<br/>(tile mappa)"]
    end

    subgraph DEVICES["📡 Dispositivi"]
        PHONE["📱 Smartphone<br/>(GPSLogger + PWA)"]
        LAPTOP["💻 Laptop<br/>(Admin)"]
        FAMILY["👨‍👩‍👧‍👦 Famiglia<br/>(Visualizzazione)"]
    end
    ...
    APP <-->|"Auth + DB"| FIREBASE
    AUTH <--> RTDB
    FUNCTIONS --> FCM
    FCM --> SW
    
    APP <-->|"Sync GPX"| DRIVE
    APP <-->|"Gap estimation"| OSRM
    APP <-->|"Meteo live"| WEATHER
    APP <-->|"Città da coord"| NOMINATIM
    UI <-->|"Tile layer"| TILES

    style CLIENT fill:#e8eaf6,stroke:#283593
    style FIREBASE fill:#fff8e1,stroke:#f57f17
    style EXTERNAL fill:#e0f7fa,stroke:#00695c
    style DEVICES fill:#fce4ec,stroke:#880e4f
```
...
```mermaid
sequenceDiagram
    participant U as 👨 Autista
    participant GL as 📱 GPSLogger
    participant QV as 🧭 Quo Vadis
    participant D as ☁️ Google Drive
    participant FB as 🔥 Firebase
    participant F as 👨‍👩‍👧‍👦 Famiglia

    Note over U,F: 🌅 MATTINA — Partenza
    U->>GL: Avvia logging (auto o manuale)
    U->>QV: Apre app → premi ▶ Start
    QV->>FB: watchPosition → salva punti live
    GL->>GL: Registra GPS ogni 30s (background)

    Note over U,F: 🚐 IN MARCIA
    QV->>FB: Aggiorna posizione ogni 5s
    FB->>F: Famiglia vede posizione live
    U->>QV: Chiude browser (usa Maps)
    Note over QV: ⚠️ Tracking live si ferma
    GL->>GL: Continua a registrare (nativo)

    Note over U,F: ⏰ DOPO 60 MINUTI
    GL->>D: Upload automatico file .gpx

    Note over U,F: 🔄 RIAPERTURA APP
    U->>QV: Riapre Quo Vadis
    QV->>QV: Rileva gap temporale
    QV->>QV: OSRM stima km mancanti
    QV->>D: Drive Sync (scarica .gpx)
    D->>QV: File GPX con punti reali
    QV->>QV: Merge + deduplicazione
    QV->>FB: Salva traccia completa
    FB->>F: Mappa aggiornata con percorso completo

    Note over U,F: 🌙 SERA — Fine giornata
    U->>GL: Premi Stop → upload finale
    GL->>D: Upload .gpx completo del giorno
    U->>QV: ⏹ Fine Giornata
    QV->>QV: Chiede parcheggio notturno
    U->>QV: Inserisce km contachilometri (opzionale)
    QV->>FB: Salva odometerKm (priorità assoluta)
```
...
```mermaid
flowchart TD
    A["👤 Utente visita Quo Vadis"] --> B{"Autenticato?"}
    
    B -->|No| C["🌍 Accesso PUBBLICO<br/>Tutto il contenuto informativo:<br/>Home, Riepilogo, Itinerario,<br/>Giorni, Luoghi, Cibo, Cultura,<br/>Attività, Zaino, Piano, Mappa"]
    
    B -->|Sì| D{"Ruolo?"}
    
    D -->|"Owner<br/>(organizzatore)"| E["🛠️ Accesso COMPLETO<br/>Tutto il pubblico +<br/>Pannello Admin<br/>Tracking controls<br/>Modifica km / Parcheggi<br/>Drive Sync config<br/>Diario (scrittura)<br/>Chat (scrittura)"]
    
    D -->|"Approved<br/>(familiare)"| F["👁️ Pubblico + Interazione<br/>Posizione live<br/>Statistiche avanzate<br/>Diario (lettura)<br/>Chat (lettura + scrittura)<br/>Notifiche Push"]
    
    D -->|"Pending<br/>(in attesa)"| G["⏳ Come pubblico<br/>+ In attesa approvazione"]
    
    D -->|"Banned"| H["🚫 Accesso Negato"]

    G -->|"Admin approva"| F
    
    E -->|"Firebase Rules"| I["🔥 DB: read + write completo"]
    F -->|"Firebase Rules"| J["🔥 DB: read + chat write"]
    C -->|"Nessun DB"| K["📄 Solo contenuto statico<br/>(HTML/JS locale)"]
    ...
    style K fill:#f5f5f5,stroke:#616161
```
...
flowchart LR
    subgraph TRIGGERS["🎯 Trigger"]
        T1["📨 Nuovo messaggio chat"]
        T2["📖 Nuova entry diario"]
        T3["📅 Reminder giornaliero"]
        T4["🔔 Countdown partenza"]
        T5["👤 Nuovo utente pending"]
        T6["🌙 Reminder tappa domani"]
    end

    subgraph BACKEND["⚡ Cloud Functions"]
        CF["sendPushNotification()"]
        FILTER["filterByNotifPrefs()"]
        TOKENS["collectTokens()"]
    end

    subgraph DELIVERY["📬 Consegna"]
        FCM["Firebase Cloud Messaging"]
        SW["Service Worker"]
        NOTIF["🔔 Notifica Push<br/>(titolo + corpo + azione)"]
    end

    subgraph USERS["👥 Destinatari"]
        OWNER["🛠️ Owner"]
        APPROVED["👁️ Approved"]
        ALL["🌍 Tutti"]
    end
    ...
    style TRIGGERS fill:#e8eaf6,stroke:#283593
    style BACKEND fill:#fff3e0,stroke:#e65100
    style DELIVERY fill:#e0f2f1,stroke:#004d40
